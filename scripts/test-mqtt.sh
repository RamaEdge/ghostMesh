...
# Test basic connectivity using containerized mosquitto tools
echo "Testing basic connectivity..."
MOSQUITTO_CONTAINER=$(podman ps --format "{{.Names}}" | grep ghostmesh-mosquitto | head -1)
if [ -z "$MOSQUITTO_CONTAINER" ]; then
    echo "Error: Could not find mosquitto container"
    exit 1
fi

if podman exec $MOSQUITTO_CONTAINER mosquitto_pub -h localhost -p 1883 -u iot -P iotpass -t "test/connectivity" -m "Hello GhostMesh" -q 1; then
...
if podman exec $MOSQUITTO_CONTAINER mosquitto_pub -h localhost -p 1883 -u gateway -P gatewaypass -t "factory/A/press01/temperature" -m '{"value": 25.5, "unit": "C"}' -q 1; then
...
if podman exec $MOSQUITTO_CONTAINER mosquitto_pub -h localhost -p 1883 -u detector -P detectorpass -t "alerts/press01/temperature" -m '{"severity": "medium", "reason": "test"}' -q 1; then
...
if podman exec $MOSQUITTO_CONTAINER mosquitto_pub -h localhost -p 1883 -u dashboard -P dashboardpass -t "control/press01/isolate" -m '{"command": "isolate", "reason": "test"}' -q 1; then
...
if podman exec $MOSQUITTO_CONTAINER mosquitto_pub -h localhost -p 1883 -u policy -P policypass -t "audit/actions" -m '{"action": "test", "result": "success"}' -q 1; then
...
if podman exec $MOSQUITTO_CONTAINER mosquitto_pub -h localhost -p 1883 -u explainer -P explainerpass -t "explanations/test-alert" -m '{"text": "Test explanation", "confidence": 0.8}' -q 1; then
